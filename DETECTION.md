...
The Renderer takes a smooth plan and recreates the uneven integer reports of a
person moving their mouse, with the texture of their hand and setup. To test that
job without mixing in Planner error, I gave it a smooth version of the real human
future. A five-report triangular average,
...
reports ending at B. Those reports are its complete observed history, and all
future raw reports are reserved for evaluation.
...
The cold and warm study tests the complete Static Planner→Renderer pipeline.
Each frozen movement is passed through both Planner seeds, and each plan is rendered
twice. That gives four complete
...
movement from 2 of the 6 unseen humans. It therefore offered no safe way to
...
realistic way to catch ABCurves while keeping false accusations of human movement
acceptably low.** The cold judges either missed the generated movements or accused
genuine humans. The warm signal relied on clean movement from the exact same
session, and even then the separation came with false positives and depended on a
baseline that ordinary changes in a person's hand and setup can disturb.
...
The cold and warm numbers use the complete Static pipeline on the same frozen B80
...
The measured Renderer state comes from those exact per-event windows. A rolling
observer or a reusable representative profile supplies a different initialization.
A separate one-draw probe compared profile alignment and found overlapping
uncertainty intervals. Its methods and results are in the
[Renderer profile sensitivity receipt](results/inference/renderer_profile_sensitivity.json).
The values here are descriptive point estimates on this ten-session panel, without
...
The raw static recordings are release attachments. The exact frozen cohort, cuts,
panel and generation schedules are included as a compact recipe. Follow
[DETECTION_REPRODUCTION.md](docs/DETECTION_REPRODUCTION.md) to rebuild the measured
inputs and generated streams with the authenticated public runtime, then run these
judges. The general builders above are for new experiments and smoke checks. Their
fresh selection is not a substitute for the frozen study recipe.
